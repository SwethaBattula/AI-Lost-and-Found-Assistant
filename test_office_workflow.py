...
os.environ["DATABASE_URL"] = "sqlite:///./test_office_workflow.db"
...
async def run_office_workflow_test():
    ...
    try:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://127.0.0.1:8000") as client:
            ...
            assert handover_res.json()["status"] == "handed_over" or handover_res.json()["status"] == "collected"
            print("  [OK] Match status updated to 'handed_over' / 'collected' (Case Closed).")

        print("\n========================================================================")
        print("      OFFICE SUPERVISION WORKFLOW TEST PASSED SUCCESSFULLY!            ")
        print("========================================================================")
    finally:
        test_db = os.path.abspath(os.path.join(os.path.dirname(__file__), "backend", "test_office_workflow.db"))
        if os.path.exists(test_db):
            try:
                os.remove(test_db)
                print("  [OK] Isolated test database cleaned up.")
            except Exception:
                pass
