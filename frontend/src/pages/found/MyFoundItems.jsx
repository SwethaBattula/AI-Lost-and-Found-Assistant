...
import { Search, PlusCircle, CheckSquare, X } from 'lucide-react';
...
const MyFoundItems = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const data = await foundItemService.getFoundItems(true); // my_items_only = true
      setItems(data);
    } catch (err) {
      console.error('Failed to fetch found items:', err);
      showToast('Failed to load your found items list.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditSubmit = async (payload) => {
    try {
      ...
      await foundItemService.updateFoundItem(editingItem.id, payload);
      showToast('Found item updated successfully.', 'success');
      setEditingItem(null);
      fetchItems();
    } catch (err) {
      console.error('Failed to update found item:', err);
  ...
  const handleDeleteConfirm = async () => {
    try {
      setActionLoading(true);
      await foundItemService.deleteFoundItem(deletingItem.id);
      showToast('Found item deleted successfully.', 'success');
      setDeletingItem(null);
      fetchItems();
    } catch (err) {
      console.error('Failed to delete found item:', err);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight">My Found Items</h2>
          <p className="text-slate-400 text-sm">Manage items you have found and registered</p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={CheckSquare}
          title={searchQuery ? 'No matching items found' : 'No found items registered yet'}
          description={
            searchQuery
              ? `No items matched "${searchQuery}". Try adjusting your search keywords.`
              : 'You have not registered any found items yet.'
          }
          actionText={searchQuery ? 'Clear Search' : 'Report Found Item'}
          actionLink={searchQuery ? null : '/found-items/new'}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-2xl w-full p-6 sm:p-8 space-y-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center pb-4 border-b border-slate-800">
              <h3 className="text-lg font-bold text-white">Edit Found Item</h3>
              <button
      ...
      <ConfirmationModal
        isOpen={!!deletingItem}
        title="Delete Found Item Record"
