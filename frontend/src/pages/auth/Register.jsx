...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.detail || 'Failed to register account. Email may already be in use.';
      setError(msg);
      showToast(msg, 'error');
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center p-4">
      <div className="w-full max-w-md space-y-8 bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-2xl">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-slate-400">Join the AI Lost & Found platform</p>
        </div>
